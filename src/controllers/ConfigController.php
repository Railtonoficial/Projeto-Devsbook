...
class ConfigController extends Controller
{
    ...
    public function save()
    {
        ...
        $passwordConfirm = filter_input(INPUT_POST, 'passwordConfirm'); // Novo campo para confirmação de senha
        ...
        if ($name && $email) {
            ...
            if (!empty($newPassword)) {
                if ($password === $passwordConfirm) { // Verificação da senha e confirmação
                    $updateFields['password'] = password_hash($newPassword, PASSWORD_DEFAULT);
                } else {
                    $_SESSION['flash'] = 'As senhas não batem. Por favor, tente novamente!';
                    $this->redirect('/config');
                }
            }
    ...
    private function cutImage($file, $w, $h, $folder)
    {
        ...
        $fileName = md5(time() . rand(0, 10000)) . '.jpg';
